...
export default function ProductPage() {
  ...
  useEffect(() => {
    async function fetchProductData() {
      const response = await API.getProductDetails(String(id));
  ...
  return (
    <>
      ...
      <div className="p-2 w-full">
        <div className="prose dark:prose-invert w-full max-w-none" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(description) }}/>
      </div>
    </>
  );
}
